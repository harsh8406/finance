...
function DonutChart({ segments, size = 120, dark }) {
  ...
  const paths = segments.map((seg) => {
    ...
    return {
      d: `M ${cx + r * Math.cos(s)} ${cy + r * Math.sin(s)} A ${r} ${r} 0 ${pct > 0.5 ? 1 : 0} 1 ${cx + r * Math.cos(e)} ${cy + r * Math.sin(e)}`,
      color: seg.color,
    };
  });
  return (
...
function makeStyles(dark) {
  ...
  return {
    ...
    navItem: (a) => ({
      ...
      borderLeft: a ? '2px solid #FF6B6B' : '2px solid transparent',
      background: a ? 'rgba(255,107,107,0.08)' : 'transparent',
      color: a ? '#FF6B6B' : textMuted,
      fontSize: 14, fontWeight: 500, transition: 'all 0.2s', marginBottom: 2,
    }),
    ...
    btn: (p)    => ({
      background: p ? '#FF6B6B' : btnSecBg,
      color: p ? '#fff' : btnSecColor,
      border: p ? 'none' : btnSecBorder,
...
export default function Dashboard({ theme, toggleTheme }) {
  ...
  // Date restrictions — today and past 5 days only
  ...
  const handleSubmit = async () => {
    ...
    if (!form.amount || Number(form.amount) <= 0) {
      setAmountError('⚠️ Amount must be greater than ₹0');
      return;
    }
    if (form.date < minDate || form.date > maxDate) {
      alert(`Date must be within the last 5 days (${minDate} to ${maxDate}). Future dates are not allowed.`);
      return;
    }
    try {
      if (editId) {
    ...
    <>
      ...
      <div style={S.app}>
        ...
        <div style={S.sidebar}>
          ...
          <button style={{ ...S.btn(false), margin: '0 16px', justifyContent: 'center' }} onClick={logout}>
            Sign Out
          </button>
        </div>
            ...
            <div>
              ...
              <div style={S.grid}>
                {[
                  ...
                  { label: 'Remaining',     value: formatINR(Math.max(0, remaining)),             sub: `${(100 - budgetPct).toFixed(0)}% left`,  color: '#4ECDC4' },
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 8 }}>
                    <div style={{ position: 'relative' }}>
                      <DonutChart dark={dark} size={100} segments={[
                        { color: budgetPct > 85 ? '#FF6B6B' : '#4ECDC4', value: totalSpent },
                        { color: dark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.07)', value: Math.max(0, budget - totalSpent) },
                      ]} />
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, fontFamily: "'Syne', sans-serif", color: S.text }}>
                        {budgetPct.toFixed(0)}%
                      </div>
                    </div>
                    <div>
                      ...
                      <div style={{ fontSize: 13, color: '#4ECDC4', marginTop: 4 }}>Left: {formatINR(Math.max(0, remaining))}</div>
                    </div>
                  </div>
                </div>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                <div>
                  ...
                  <div style={S.sub}>{filtered.length} transactions</div>
                </div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                {[{ id: 'all', label: 'All', icon: '' }, ...CATEGORIES].map(c => (
                  <button key={c.id} onClick={() => setFilterCat(c.id)} style={{ ...S.btn(filterCat === c.id), padding: '7px 14px', fontSize: 12 }}>
                    {c.icon} {c.label?.split(' ')[0] || c.label}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
                ...
                {['date', 'amount', 'title'].map(s => (
                  <button key={s} onClick={() => setSortBy(s)} style={{ ...S.btn(sortBy === s), padding: '5px 12px', fontSize: 12 }}>
              ...
              {filtered.length === 0 && (
                <div style={{ textAlign: 'center', padding: '40px', color: S.textMuted, fontSize: 13 }}>
                    ...
                    <div key={c.id} style={{ marginBottom: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 13 }}>
                          <span>{c.icon}</span>
                          <span style={{ fontWeight: 500, color: S.text }}>{c.label}</span>
                        </div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: c.color }}>
                          {formatINR(total)} <span style={{ color: S.textMuted, fontWeight: 400 }}>({pct.toFixed(1)}%)</span>
                        </div>
                      </div>
          ...
          {view === 'budget' && (
            <div>
              ...
              <div style={{ ...S.card, maxWidth: 520, marginBottom: 20 }}>
                ...
                <div style={{ fontSize: 36, fontFamily: "'Syne', sans-serif", fontWeight: 800, color: budgetPct > 85 ? '#FF6B6B' : '#4ECDC4', marginBottom: 16 }}>
                  {formatINR(budget)}
                </div>
                ...
                <input
                  ...
                  style={{
                    ...
                    height: 4,
                    background: `linear-gradient(to right, #FF6B6B ${budgetPct}%, ${dark ? 'rgba(255,255,255,0.15)' : '#e2e8f0'} ${budgetPct}%)`,
                    borderRadius: 2, outline: 'none', cursor: 'pointer', marginBottom: 8,
                  }}
                />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: S.textMuted, marginBottom: 16 }}>
                ...
                <div style={S.progressBar}>
                  <div style={{ height: '100%', width: `${budgetPct}%`, background: progressColor(budgetPct), borderRadius: 3, transition: 'width 0.6s ease' }} />
                </div>
                ...
                {budgetPct > 85 && (
                  <div style={{ marginTop: 16, background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.3)', borderRadius: 10, padding: '12px 14px', fontSize: 13, color: '#FF6B6B' }}>
                    ⚠️ You've used {budgetPct.toFixed(0)}% of your budget. Consider cutting back on spending.
                  </div>
                )}
              </div>
                ...
                return (
                  <div key={c.id} style={{ ...S.card, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={S.progressBar}>
                        <div style={{ height: '100%', width: `${Math.min(100, pct)}%`, background: c.color, borderRadius: 3, transition: 'width 0.6s ease' }} />
                      </div>
                    </div>
        ...
        {showForm && (
          <div style={S.formOverlay} onClick={e => { if (e.target === e.currentTarget) setShowForm(false); }}>
            <div style={S.formCard}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 20, marginBottom: 24, color: S.text }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input style={S.input} placeholder="e.g. Zomato Order" value={form.title}
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    style={{ ...S.input, borderColor: amountError ? '#FF6B6B' : undefined }}
                  ...
                  />
                  {amountError && (
                    <div style={{ fontSize: 11, color: '#FF6B6B', marginTop: 5 }}>{amountError}</div>
                  )}
                </div>
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    value={form.date} min={minDate} max={maxDate}
                    onChange={e => setForm(p => ({ ...p, date: e.target.value }))}
                  />
                  <div style={{ fontSize: 11, color: S.textMuted, marginTop: 5 }}>
                    Last 5 days only · No future dates
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <select style={{ ...S.input, cursor: 'pointer' }} value={form.category}
                  ...
                  {CATEGORIES.map(c => (
                    <option key={c.id} value={c.id} style={{ background: dark ? '#111' : '#fff', color: dark ? '#fff' : '#1a1a2e' }}>
                      {c.icon} {c.label}
                    </option>
                  ))}
                </select>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <input style={S.input} placeholder="Add a note..." value={form.note}
                  onChange={e => setForm(p => ({ ...p, note: e.target.value }))} />
              </div>

              {/* Buttons */}
              <div style={{ display: 'flex', gap: 10 }}>
                <button style={{ ...S.btn(false), flex: 1, justifyContent: 'center' }} onClick={() => setShowForm(false)}>
                  Cancel
                </button>
                <button style={{ ...S.btn(true), flex: 1, justifyContent: 'center' }} onClick={handleSubmit}>
